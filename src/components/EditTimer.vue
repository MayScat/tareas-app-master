    ...
    <v-dialog persistent scrollable v-model="dialog" max-width="850px">
        <v-card>
            ...
            <v-card-actions>
                ...
                <v-btn v-if="end_date === ''" color="warning darken-2" @click="insertTimer(1)">Save & Continue</v-btn>
    ...
    export default {
        ...
        methods: {
            openModal(){
                ...
                this.time2 = this.timeStopped
                this.date2 = this.dateStopped
            },
            getTypes(){
            ...
            insertTimer() {
                if(this.time != '' & this.time2 != ''){
                    ...
                    if(diff >= 18000){
                        ...
                        this.apiCall("update-timer-config", {timer_id: this.timer_id, start_date: this.date, start_time: this.time, end_date: this.date2, end_time: this.time2, type: this.type, details: this.details }).then((response) => {
                            if(response.success) {
                                Events.$emit('reload')
                                this.close()
                            } else {
