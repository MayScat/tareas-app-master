          ...
          <v-container>
            <v-row dense>
              ...
              <v-col cols="12" md="6" sm="12">
                <v-select :items="statuses" v-model="status" outlined class="ma-0 pa-0" label="Status"></v-select>
  ...
  export default {
    ...
    data() {
      return {
        ...
        color: '',
        members: [],
        statuses: [{text: "Open", value: 0},{ text: "Closed", value: 1}],
        status: 0,
        users: [],
        project: '',
        subProject: '',
    ...
    methods: {
      ...
      openModal() {
        this.dialog = true
        this.members = this.ticket.members.split(",")
        for(let i = 0; i < this.members.length; i++){
          this.members[i] = parseInt(this.members[i])
        }
        this.client = this.ticket.client_id
        this.project = this.ticket.project_id
        this.name = this.ticket.name
        this.status = this.ticket.status
        this.subProject = this.ticket.sub_project_id
      ...
      insertTicket() {
        this.apiCall("edit-ticket", {
          ...
          if (response.success == true) {
            this.color = 'success'
            this.errors = ['Ticket has been succesfully updated.']
            setTimeout(() => {
              this.close()
              Events.$emit('reload')
            }, 950);
