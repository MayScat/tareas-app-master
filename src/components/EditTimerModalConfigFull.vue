<template>
<div>
    <b style="text-decoration: underline; font-size: 16px; color: #1976d2; cursor: pointer;" @click="openModal">{{label}}</b>
    <v-dialog persistent scrollable v-model="dialog" max-width="850px">
        <v-card>
            ...
            <v-card-actions>
                ...
                <v-btn v-if="date2 == '' || time2 == ''" color="warning darken-2" @click="insertTimer(1)">Save & Continue</v-btn>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            insertTimer(cont) {
                if(this.time != ''){
                    this.apiCall("edit-timer-config-full", {cont: cont,timer_id: this.timer_id, task_id: this.selectedTask, client_id: this.selectedClient, project_id: this.selectedProject, start_date: this.date, start_time: this.time, end_date: this.date2, end_time: this.time2, type: this.type, details: this.details }).then((response) => {
                        if(response.success) {
                            Events.$emit('reload')
                            this.close()
                        } else {
