<template>
<div>
<v-icon color="success" big @click="openModal">mdi-pencil</v-icon>
...
<script>
  export default {
    ...
    methods: {
        openModal(){
            this.dialog = true
            this.apiCall('get-holiday-info', {id: this.holiday}).then((res) => {
                if (res.success) {
                    this.hol = res.holiday
                }
            }).catch((error) => {
                    console.log(error)
            })
        ...
        editHoliday(){
            this.apiCall("edit-holiday", {
              ...
              if(response.success == true) {
                this.color = 'success'
                this.errors = ['Holiday has been succesfully edited.']
                setTimeout(() => {
                  this.dialog = false
                  Events.$emit('reload')
                  this.errors = []
